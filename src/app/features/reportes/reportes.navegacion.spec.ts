...
/**
 * Navegación al módulo de Reportes: ruta, menú y guard (RBAC de
...
describe('Menú lateral', () => {
 const item = () =>
 MODULES.flatMap((m) => m.items.map((i) => ({ modulo: m, item: i }))).find((x) => x.item.cus === '');

 it('el ítem de apunta a /reportes con nombre corto Reportes', () => {
