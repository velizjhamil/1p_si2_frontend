...
/**
 * Carrito de Compras (estado global con signals).
 *
 ...
 * consume el endpoint real POST /api/v1/ventas/checkout del
...
interface VentaDTO {
 ...
 /** tipo de venta (ONLINE | POS). Default ONLINE. */
...
export class CarritoService {
 ...
 // ------------------------------------------------------------------- CRUD
 /**
 * Agrega una variante (producto+talla+color) al carrito.
 ...
 */
 ...
 i.talla === item.talla &&
 i.color === item.color &&
 (item.id_sucursal ? i.id_sucursal === item.id_sucursal : true),
 );
 if (idx >= 0) {
 ...
 id_sucursal: item.id_sucursal ?? copia[idx].id_sucursal,
 sucursal_nombre: item.sucursal_nombre ?? copia[idx].sucursal_nombre,
 };
 return copia;
 }
 return [...items, nuevo];
 ...
 /** Elimina un ítem por variante (producto+talla+color). */
 ...
 /** Actualiza la cantidad de una variante (mínimo 1). */
 actualizarCantidad(
 producto_id: number,
 talla: string,
 color: string,
 ...
 /** Sincroniza el carrito local con el backend persistente de base de datos. */
 ...
 // ------------------------------------------------------------- checkout
 /**
 * Procesa la compra contra el backend real (flujo ONLINE del
 ...
 */
 ...
 /**
 * Procesa una venta POS (mostrador) por POST /api/v1/ventas/pos
 ...
 * con la forma extendida de y delega al endpoint.
 */
 ...
 const sucursalItem = items.find((i) => i.id_sucursal);
 if (sucursalItem?.id_sucursal) {
 body['id_sucursal'] = sucursalItem.id_sucursal;
 }
